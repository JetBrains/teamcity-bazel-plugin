...
class MessageFactoryImplTest {
    ...
    private val messageFactory: MessageFactory = MessageFactoryImpl()

    @Test(dataProvider = "buildProblemDistinctInputs")
    fun shouldGenerateDistinctBuildProblemIdentities(projectId1: String, errorId1: String,
                                                     projectId2: String, errorId2: String) {
    ...
    fun shouldGenerateSameBuildProblemIdentities() {
        val buildProblem1 = messageFactory.createBuildProblem(
                "problem description 1", "same project", "same error")
        val buildProblem2 = messageFactory.createBuildProblem(
                "problem description 2", "same project", "same error")
        Assert.assertEquals(
                buildProblem1.attributes["identity"],
                buildProblem2.attributes["identity"])
