...
class ProgressHandler : EventHandler {
    ...
    override fun handle(ctx: ServiceMessageContext) =
            ...
            if (event.stderr.isNotBlank()) {
                ...
                val mostSevereLevel = decomposedMessages
                    .map { it.level }
                    .maxByOrNull { it.value } ?: LogLevel.Normal

                for (errItem in decomposedMessages) {
                    val messageText = ctx.buildMessage()
                        .append(errItem.text)
                        .toString()

                    val message = ctx.messageFactory.let {
    ...
    private enum class LogLevel(val value: Int, val defaultColor: Color) {
        Error(3, Color.Error),
    ...
    companion object {
        private val prefixConfigs = listOf(
            ...
            MessageConfig("^(\\[\\s*\\d+\\s*\\/\\s*\\d+\\s*\\])\\s*\\[\\s*\\-+\\s*\\]\\s*(.+)\$".toRegex(), LogLevel.Normal, Color.Success),
            MessageConfig("^(\\[\\s*\\d+\\s*\\/\\s*\\d+\\s*\\])\\s*(.+)\$".toRegex(), LogLevel.Normal, Color.Success)
        )
    }
}
